...
const OUTCOME_STYLE: Record<string, { color: string; bg: string; border: string }> = {
  "Accepted":  { color: "#14532D", bg: "#F0FDF4", border: "#BBF7D0" },
  "Rejected":  { color: "#7B1D1D", bg: "#FEF2F2", border: "#FECACA" },
  "Withdrawn": { color: "#5C4A1E", bg: "#FFFBEB", border: "#FDE68A" },
}
...
export default function DelayDashboard({
  ...
  const active   = [...enriched]
    .filter(e => !TERMINAL_STATUSES.has(e.submission.timeline[e.submission.timeline.length - 1].status))
  ...
  const highRisk  = active.filter(e => e.risk_level === "high" || e.risk_level === "severe").length
  const avgDays   = active.length
    ? Math.round(active.reduce((a, b) => a + b.days_since_submission, 0) / active.length) : 0
  ...
  return (
    <div>
      ...
      {/* Stats — based on active submissions only */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 24 }}>
        {[
          { label: "Active submissions",  value: active.length,                     color: "var(--navy)" },
      ...
      {resolved.length > 0 && (
        <div style={{ marginTop: active.length > 0 ? 32 : 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
            <span className="section-label" style={{ marginBottom: 0 }}>Resolved</span>
            <span style={{ fontSize: 11, color: "var(--ink-muted)", background: "var(--surface-alt)",
              border: "1px solid var(--linen-border)", borderRadius: 10, padding: "1px 8px" }}>
              {resolved.length}
            </span>
          </div>
          {resolved.map(sub => {
            const latest  = sub.submission.timeline[sub.submission.timeline.length - 1]
            const outcome = OUTCOME_STYLE[latest.status] ?? OUTCOME_STYLE["Withdrawn"]
            return (
              <div key={sub.submission.id} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ flex: 1, minWidth: 0, marginRight: 12 }}>
                    <button
                      onClick={() => onSelectSubmission(sub.submission.id)}
                      style={{ fontFamily: "var(--font-serif)", fontSize: 15, color: "var(--navy)",
                        background: "none", border: "none", cursor: "pointer", padding: 0,
                        textAlign: "left", textDecoration: "underline", textDecorationColor: "transparent",
                        transition: "text-decoration-color 0.15s",
                      }}
                    ...
                    <div style={{ display: "flex", gap: 12, marginTop: 6, flexWrap: "wrap", alignItems: "center" }}>
                      <span style={{
                        ...
                        color: outcome.color, background: outcome.bg, border: `1px solid ${outcome.border}`,
                      }}>
                        {latest.status.toUpperCase()}
                      </span>
                      <span style={{ fontSize: 11, color: "var(--ink-muted)" }}>
                        {sub.days_since_submission}d total
                      </span>
                      <span style={{ fontSize: 11, color: "var(--ink-muted)" }}>
                        Submitted {formatDate(sub.submission.timeline[0].date)}
                      </span>
